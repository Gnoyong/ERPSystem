	...
	<section class="rt_wrap content mCustomScrollbar">
		<div class="rt_content">
			<div class="page_title">
				<h2 class="fl">出库单审核</h2>
			</div>
			<section class="mtb">
			<form action="tzcksh" method="post">
				...
				<tbody id="list">
					<c:forEach items="${list}" var="list" varStatus="num">
						<tr>
							...
							<td id="hw" class="center">${list.hw}</td>
							<td id="name" class="center">${list.name}</td>
							<td id="phone" class="center">${list.phone}</td>
							<td id="money" class="center">${list.money}</td>
							<td id="jg" class="center">${list.jg}</td>
							<td id="sl" class="center">${list.sl}</td>
							<td id="lr" class="center">${list.lr}</td>
							<td id="zrr" class="center">${list.zrr}</td>
							<td id="date" class="center">${list.date}</td>
							<td class="center"> 
							<a href="${pageContext.request.contextPath}\user\cksh?id=${list.id}">
							<input type="button" class="link_btn" value="通过" >
								</a>
								<a href="${pageContext.request.contextPath}\user\sb?id=${list.id}">
